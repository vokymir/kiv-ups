...
class ResourceManager:
    ...
    def __init__(self):
        self.cache = {}
        self.placeholder_color = "#3498db"

    def get_card_image(self, card_name, width, height):
        """
        Loads image from assets/card_name.jpg.
        If not found, creates a generated placeholder.
        """
        key = f"{card_name}_{width}_{height}"
        if key in self.cache:
            return self.cache[key]

        path = os.path.join(config.ASSETS_DIR, f"{card_name}.jpg")
        ...
        try:
            ...
            # Create a placeholder image if file missing
            img = Image.new('RGB', (width, height), color=self.placeholder_color)
            d = ImageDraw.Draw(img)
            d.text((10, 10), card_name[:4], fill="white")
...
class PrsiApp(tk.Tk):
    def __init__(self, controller):
        ...
        for F in (LoginFrame, LobbyFrame, GameFrame):
...
class LoginFrame(tk.Frame):
    def __init__(self, parent, app):
        ...
        center_frame = tk.Frame(self, bg=config.BG_COLOR)
        ...
        self.user_var = tk.StringVar(value="Player1")
        ...
        btn = tk.Button(center_frame, text="CONNECT", font=("Arial", 12, "bold"),
                        bg=config.TABLE_COLOR, fg="white", width=20, height=2,
                        command=self.on_connect)
        btn.pack(pady=20)

    def _create_entry(self, parent, label, var):
...
class LobbyFrame(tk.Frame):
    def __init__(self, parent, app):
        ...
        list_container = tk.Frame(self, bg=config.BG_COLOR)
    ...
    def _create_room_row(self, room):
        ...
        info_text = f"Room #{room['id']} | State: {room['state']} | Players: {room.get('players', '?')}"
        tk.Label(row, text=info_text, font=("Arial", 12), bg="white").pack(side="left", padx=20)

        if room['state'] == "OPEN":
            btn = tk.Button(row, text="JOIN", bg=config.TABLE_COLOR, fg="white",
...
class GameFrame(tk.Frame):
    def __init__(self, parent, app):
        ...
        top_bar = tk.Frame(self, bg="#1e8449", height=100)
        ...
        # Opponent Area
        self.opponent_area = tk.Frame(top_bar, bg="#1e8449")
        self.opponent_area.pack(side="top", pady=5)
        self.lbl_opponent_name = tk.Label(self.opponent_area, text="Waiting...", bg="#1e8449", fg="white", font=("Arial", 12, "bold"))
        ...
        middle_area = tk.Frame(self, bg=config.TABLE_COLOR)
    ...
    def set_opponent_hand(self, count):
        """Draws card backs for opponent."""
        for w in self.opponent_cards_frame.winfo_children(): w.destroy()

        # Using a generic 'back' image or just a rectangle
        img = self.app.resources.get_card_image("back", 50, 70) # Smaller for opponent

        for _ in range(count):
            lbl = tk.Label(self.opponent_cards_frame, image=img, bg="#1e8449")
            lbl.image = img
            lbl.pack(side="left", padx=2)

    def set_table_card(self, card_name):
        img = self.app.resources.get_card_image(card_name, config.CARD_WIDTH, config.CARD_HEIGHT)
        self.lbl_played.config(image=img, text="")
        self.lbl_played.image = img

    def set_deck_visible(self, visible=True):
        if visible:
            img = self.app.resources.get_card_image("back", config.CARD_WIDTH, config.CARD_HEIGHT)
            self.lbl_deck.config(image=img, text="")
            self.lbl_deck.image = img
            # Add click to draw
            self.lbl_deck.bind("<Button-1>", lambda e: self.app.controller.request_draw_card())
        else:
            self.lbl_deck.config(image="", text="Empty")

    def update_player_hand(self, card_names):
        """
        ...
        for card in card_names:
            img = self.app.resources.get_card_image(card, config.CARD_WIDTH, config.CARD_HEIGHT)
            btn = tk.Button(self.player_cards_frame, image=img, bg="#1e8449", borderwidth=0,
...
class RulesWindow(tk.Toplevel):
    def __init__(self, parent, resources):
        ...
        for card_type, desc in config.RULES_TEXT.items():
            row = tk.Frame(scroll_frame, pady=10, padx=10, relief="groove", borderwidth=1)
            row.pack(fill="x", padx=10, pady=5)

            # Left: Image placeholder (or real image if you have a representative one)
            # Just using a text label for type here, but could use resources.get_card_image
